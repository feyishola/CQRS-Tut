...
builder.Services.AddMediatR(cfg => cfg.RegisterServicesFromAssembly(typeof(Program).Assembly)); // this registers MediatR services and handlers from the assembly containing the Program class. This allows us to use MediatR for handling commands, queries, and events in a more decoupled and organized way. By registering the services from the assembly, MediatR will automatically discover and register all handlers that implement the appropriate interfaces (e.g., IRequestHandler for commands and queries, INotificationHandler for events) without needing to manually register each handler. This can help simplify the setup of our application and make it easier to manage our handlers as our application grows.
